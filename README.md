...
这是一个完整可运行的 Python 3.12 项目，用于每个工作日北京时间 10:00 通过 Cloudflare Worker 触发 GitHub Actions，采集最近 24 小时跨境电商新闻并推送钉钉；GitHub Actions 自带 schedule 会在 10:00-10:55 做后备补偿，并用当天 run-key 防止重复发送。程序会硬性排除超过 7 天的旧闻，按热度和来源优先级发送 TOP10；内容不足时按备用策略补位，仍不足则不硬凑。
...
4. Cloudflare Worker 按工作日北京时间 `10:00` 触发 GitHub Actions；GitHub Actions 自带 schedule 在 `10:00-10:55` 后备补偿。GitHub Actions 可能会有排队延迟，但会按当天 `run-key` 防止重复发送。
